...
function convertLine(line, nextLine) {
  ...
  } else if (isLegacyAttributeInterpolation(line)) {
    return convertLegacyAttributeInterpolation(line);
...
function isLegacyAttributeInterpolation(line) {
  const lineTrimedLeft = line.trimLeft();
  const regex = /^(\w+)([(])(\w.*,\ )*(\w.*)([)])(.*)$/g;
  let matches = getMatches(lineTrimedLeft, regex, 1);
  if (matches === null) {
    return false;
  }
  const [ tag, openingBrace, ...rest ] = matches;
  const attibutes = rest.slice(0, rest.length - 2);

  for (const attibute of attibutes) {
    if (attibute.includes('#{')) {
      return true;
    }
  }
  return false;
...
function convertLegacyAttributeInterpolation(line) {
  const lineTrimedLeft = line.trimLeft();
  const indentionLength = line.length - lineTrimedLeft.length;
  ...
  let matches = getMatches(lineTrimedLeft, regex, 1);
  const [ tag, openingBrace, ...rest ] = matches;
  let attibutes = rest.slice(0, rest.length - 2);
  const closingBrace = rest.slice(rest.length - 2, rest.length - 1)[0];
  const andAttributes = rest.slice(rest.length - 1)[0];

  attibutes = attibutes.map((attibute) => {
    if (attibute.includes('#{')) {
      return attibute.split('"').join('\`').split('\'').join('\`').split('#{').join('${');
    }
    return attibute;
  })
  return `${indention}${tag}${openingBrace}${attibutes.join('')}${closingBrace}${andAttributes}`;
}
...
function getMatches(string, regex) {
  ...
  const filteredMatches = matches.filter((match) => match !== undefined);
